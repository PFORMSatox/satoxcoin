...
`./`               | `satoxcoin.conf`     | User-defined [configuration settings](satoxcoin-conf.md) for `satoxcoind` or `satoxcoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `satoxcoind.pid`     | Stores the process ID (PID) of `satoxcoind` or `satoxcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `satoxcoind` or `satoxcoin-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [satoxcoin.conf](satoxcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
This table describes the files installed by Satoxcoin Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| satoxcoin.conf                                             | [Generated](../contrib/devtools/gen-bitcoin-conf.sh) configuration file     |
| bin/satoxcoin                                              | Command-line tool for interacting with Satoxcoin. Calls other binaries below. |
| bin/satoxcoin-cli                                          | Tool for making node and wallet RPC calls.                                  |
| bin/satoxcoin-qt                                           | Satoxcoin node and wallet GUI                                               |
| bin/satoxcoin-tx                                           | Tool for creating and modifying transactions                                |
| bin/satoxcoin-util                                         | Miscellaneous utilities                                                     |
| bin/satoxcoin-wallet                                       | Satoxcoin wallet tool                                                       |
| bin/satoxcoind                                             | Satoxcoin node and wallet daemon                                            |
| *lib/libsatoxcoinconsensus.so*                            | Shared library containing core consensus and validation code                |
| *lib/pkgconfig/libsatoxcoinconsensus.pc*                  | Pkg-config metadata for linking to `libsatoxcoinconsensus`                  |
| *libexec/bench_satoxcoin*                                 | Benchmarking tool for measuring node performance                            |
| *libexec/satoxcoin-chainstate*                            | Tool to validate and connect blocks                                         |
| libexec/satoxcoin-qt                                      | IPC-enabled alternative to `satoxcoin-qt`                                   |
| libexec/satoxcoin-node                                    | IPC-enabled alternative to `satoxcoind`                                     |
| libexec/test_satoxcoin                                    | Unit test binary                                                            |
| *libexec/test_satoxcoin-qt*                               | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `satoxcoin-cli`, `satoxcoind`, and others |
