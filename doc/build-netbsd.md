...
This guide describes how to build satoxcoind, command-line utilities, and GUI on NetBSD.
...
To build Satoxcoin Core without the wallet, use `-DENABLE_WALLET=OFF`.
...
Satoxcoin Core can provide notifications via ZeroMQ. To compile ZMQ support, install the following dependency and pass `-DWITH_ZMQ=ON` when configuring.
```bash
...
There are many ways to configure Satoxcoin Core. Here is an example that
