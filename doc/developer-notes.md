...
Satoxcoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while satoxcoind is running with the `logging`
RPC.  For instance, launching satoxcoind with `-debug` or `-debug=1` will turn on
...
with "play satoxcoins" on a test network.
...
Satoxcoin Core is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at satoxcoind startup or
`satoxcoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `satoxcoin-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=test/sanitizer_suppressions/valgrind.supp build/bin/test_satoxcoin
$ valgrind --suppressions=test/sanitizer_suppressions/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_satoxcoin --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/satoxcoind -printtoconsole
...
# A coverage report will now be accessible at `./build/test_satoxcoin.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_satoxcoin \
    --object=build/bin/satoxcoind \
    ...
    --project-title="Satoxcoin Core Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Satoxcoin Core Fuzz Coverage Report"
```
...
To profile a running satoxcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep satoxcoind` -- sleep 60
```
...
Satoxcoin Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`satoxcoind`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `satoxcoind.cpp`. Responsible for starting up and
...
- Net threads:
  ...
  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 60777.
...
pay attention to for reviewers of Satoxcoin Core code.

## General Satoxcoin Core
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers (hh, h, l, ll, j, z, t, L) for arithmetic types.
  ...
  - *Rationale*: Satoxcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Normally, these are maintained by active developers of Satoxcoin Core, in which case
...
a Satoxcoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
The tool instructions also include a list of the subtrees managed by Satoxcoin Core.
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Satoxcoin Core subtree to avoid delay.
...
file descriptors, it will cause problems with Satoxcoin's `select()` loop, because
...
compatibility between nodes. This happened in Satoxcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Satoxcoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `satoxcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `satoxcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Satoxcoin Core, the wallet was always
