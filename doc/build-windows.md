...
The options known to work for building Satoxcoin Core on Windows are:
...
Note that for WSL the Satoxcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/satoxcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\satoxcoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/satoxcoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/satoxcoin --strip
```
