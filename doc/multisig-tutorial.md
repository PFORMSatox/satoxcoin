...
Currently, it is possible to create a multisig wallet using Satoxcoin Core only.
...
Before starting this tutorial, start the satoxcoin node on the signet network.

```bash
./build/bin/satoxcoind -signet -daemon
```
...
At the time of writing, there is no way to extract a specific path from wallets in Satoxcoin Core. For this, an external signer/xpub can be used.
...
All RPCs in Satoxcoin Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["xpub_${n}"]=$(./build/bin/satoxcoin rpc -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))' | sed 's /0/\* /<0;1>/* ')
done
```
...
checksum=$(./build/bin/satoxcoin rpc -signet getdescriptorinfo $desc | jq -r '.checksum')
...
Documentation for these and other parameters can be found by typing `./build/bin/satoxcoin rpc -signet help importdescriptors`.
...
```bash
./build/bin/satoxcoin rpc -signet createwallet "multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/satoxcoin rpc -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/satoxcoin rpc -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/satoxcoin rpc -signet loadwallet "participant_${n}"; done
./build/bin/satoxcoin rpc -signet loadwallet "multisig_wallet_01"
```
...
```bash
receiving_address=$(./build/bin/satoxcoin rpc -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/satoxcoin-cli -a $receiving_address
```
...
```bash
./build/bin/satoxcoin rpc -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSBT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed Satoxcoin Transaction (PSBT).

PSBT is a data format that allows wallets and other tools to exchange information about a Satoxcoin transaction and the signatures necessary to complete it. [[source](https://bitcoinops.org/en/topics/psbt/)]

The current PSBT version (v0) is defined in [BIP 174](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid satoxcoin address.
...
```bash
balance=$(./build/bin/satoxcoin rpc -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/satoxcoin rpc -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/satoxcoin rpc -signet -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/satoxcoin rpc -signet decodepsbt $funded_psbt

./build/bin/satoxcoin rpc -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/satoxcoin rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/satoxcoin rpc -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/satoxcoin rpc -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/satoxcoin rpc -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/satoxcoin rpc -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/satoxcoin rpc -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/satoxcoin rpc -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/satoxcoin rpc -signet finalizepsbt $psbt_2 | jq -r '.hex')
