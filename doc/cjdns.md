# CJDNS support in Satoxcoin Core

It is possible to run Satoxcoin Core over CJDNS, an encrypted IPv6 network that
...
## Run Satoxcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Satoxcoin Core
...
When enabled, this option tells Satoxcoin Core that it is running in an
...
IPv6 local network. This helps Satoxcoin Core perform better address management:
...
## CJDNS-related information in Satoxcoin Core

There are several ways to see your CJDNS address in Satoxcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `satoxcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `satoxcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `satoxcoin-cli help getnodeaddresses` for details.

`satoxcoin-cli` can also be used for all commands.
