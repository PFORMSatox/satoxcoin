...
`test/functional/test_framework/test_framework.py:SatoxcoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/satoxcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and satoxcoind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_satoxcoind = config["components"].getboolean("ENABLE_SATOXCOIND")
    if not (enable_wallet and enable_cli and enable_satoxcoind):
        print("No functional tests to run. Wallet, utils, and satoxcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if satoxcoind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "satoxcoind"]) is not None:
            print("%sWARNING!%s There is already a satoxcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "SATOXCOIND" not in os.environ:
        os.environ["SATOXCOIND"] = build_dir + '/src/satoxcoind' + exeext
        os.environ["SATOXCOINCLI"] = build_dir + '/src/satoxcoin-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `satoxcoin-cli help` (`rpc_interface.txt`).
