...
from test_framework.util import assert_equal, assert_is_hash_string, assert_does_not_contain_key, assert_raises_rpc_error
...
class AssetTest(BitcoinTestFramework):
    ...
    def init_wallet(self, node):
        # Satoxcoin regtest uses different address prefixes than the BTC
        # deterministic test key, so rely on each node's own keypool instead
        # of importing the Bitcoin Core deterministic key. Still create the
        # wallet so wallet RPCs are available.
        wallet_name = self.wallet_names[node]
        if wallet_name is not None:
            self.nodes[node].createwallet(wallet_name=wallet_name, load_on_startup=True)

    def activate_assets(self):
        self.log.info("Funding node[0] for asset issuance...")
        n0 = self.nodes[0]
        # Assets are always active on Satoxcoin; mine enough blocks for the
        # coinbase reward to mature (COINBASE_MATURITY = 100).
    ...
    def big_test(self):
        ...
        address0 = self.legacy_address(n0)
        ...
        address1 = self.legacy_address(n1)
        ...
        for assaddr in n0.listaddressesbyasset("MY_ASSET").keys():
            if n0.getaddressinfo(assaddr)["ismine"]:
        ...
        n0.transfer(asset_name="MY_ASSET", qty=800, to_address="Hx3ktVQY3hiWNHVKhTVPF8JtKTZXmUaeFG")
        self.generate(n0, 1)
        bal_after_burn = n0.listmyassets(asset="MY_ASSET", verbose=True)
        self.log.info("listmyassets after burn: %s", bal_after_burn)
        assert_does_not_contain_key("MY_ASSET", bal_after_burn)

        self.log.info("Calling reissue()...")
        address1 = self.legacy_address(n0)
        ...
        n0.issue("SATOXCOIN1", 1000)
        n0.issue("SATOXCOIN2", 1000)
        n0.issue("SATOXCOIN3", 1000)
        self.generate(n0, 1)
        self.sync_all()

        n0.listassets(asset="SATOXCOIN*", verbose=False, count=2, start=-2)
        ...
        satoxcoin_assets = n0.listassets(asset="SATOXCOIN*", verbose=False, count=2, start=-2)
        assert_equal(len(satoxcoin_assets), 2)
        assert_equal(satoxcoin_assets[0], "SATOXCOIN2")
        assert_equal(satoxcoin_assets[1], "SATOXCOIN3")
        self.sync_all()
    ...
    def chain_assets(self):
        ...
        chain_address = self.legacy_address(n0)
    ...
    def ipfs_state(self):
        ...
        address1 = self.legacy_address(n0)
        address2 = self.legacy_address(n0)
        ipfs_hash = "QmcvyefkqQX3PpjpY5L8B2yMd47XrVwAipr6cxUt2zvYU8"
    ...
    def db_corruption_regression(self):
        ...
        n0.issue(asset_name)
        a = self.generate(n0, 1, sync=False)[0]

        n0.reissue(asset_name, 500, self.legacy_address(n0))
        # noinspection PyStatementEffect
        self.generate(n0, 1, sync=False)[0]
    ...
    def reissue_prec_change(self):
        ...
        address = self.legacy_address(n0)

        n0.issue(asset_name, 10, "", "", 0, True, False)
        self.generate(n0, 1, sync=False)
