...
"""Test framework for satoxcoin utils.
...
def main():
    sys.exit(
        0)  # ~~ This test is to test Satoxcoin-TX which currently does not compile, so for now do not run this test, just return success so make check passes.
...
def bctest(testDir, testObj, buildenv):
    ...
    if "error_txt" in testObj:
        ...
        # That stderr is empty if no errors are expected. However, satoxcoin-tx
