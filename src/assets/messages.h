...
#ifndef SATOXCOIN_MESSAGES_H
#define SATOXCOIN_MESSAGES_H
...
// Lock for messaging
extern RecursiveMutex cs_messaging;
...
public:
    ...
    SERIALIZE_METHODS(CMessage, obj)
    {
        READWRITE(obj.out);
        READWRITE(obj.strName);
        READWRITE(obj.ipfsHash);
        READWRITE(obj.time);
        READWRITE(obj.nExpiredTime);
        READWRITE(obj.nBlockHeight);

        if constexpr (ser_action.ForRead()) {
            int8_t nStatus = 6;
            ::Unserialize(s, nStatus);
            obj.status = MessageStatusFromInt(nStatus);
        } else {
            ::Serialize(s, IntFromMessageStatus(obj.status));
        }
    }
};
