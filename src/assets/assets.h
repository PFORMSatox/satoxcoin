...
#ifndef SATOXCOIN_ASSET_PROTOCOL_H
#define SATOXCOIN_ASSET_PROTOCOL_H
...
inline const std::string NATIVE_ASSET_TICKER{"SATOX"};
...
bool CheckIssueDataTx(const CTxOut& txOut); // OP_SATOXCOIN_ASSET RVNQ (That is a Q as in Que not an O)
bool CheckOwnerDataTx(const CTxOut& txOut);// OP_SATOXCOIN_ASSET RVNO
bool CheckReissueDataTx(const CTxOut& txOut);// OP_SATOXCOIN_ASSET RVNR
bool CheckTransferOwnerTx(const CTxOut& txOut);// OP_SATOXCOIN_ASSET RVNT
...
// Deployment helpers. Assets and messaging/restricted are active on mainnet and
// testnet from their activation heights; regtest activates them immediately.
...
#endif //SATOXCOIN_ASSET_PROTOCOL_H
