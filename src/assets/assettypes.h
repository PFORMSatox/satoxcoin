...
#ifndef SATOXCOIN_NEWASSET_H
#define SATOXCOIN_NEWASSET_H
...
// Satoxcoin asset script marker bytes (must match consensus values).
#define SATOX_N 114
#define SATOX_E 118
#define SATOX_X 110
#define SATOX_Q 113
...
// Script classification types for satoxcoin asset outputs (must match
// consensus values from the original satoxcoin codebase).
static const int TX_NEW_ASSET = 8;
static const int TX_REISSUE_ASSET = 9;
static const int TX_TRANSFER_ASSET = 10;
static const int TX_RESTRICTED_ASSET_DATA = 11;
...
const uint8_t IPFS_SHA2_256 = 0x12;
const uint8_t TXID_NOTIFIER = 0x54;
const uint8_t IPFS_SHA2_256_LEN = 0x20;

template <typename Stream, typename Operation, typename StringT>
bool ReadWriteAssetHash(Stream &s, Operation ser_action, StringT& strIPFSHash)
{
...
public:
    ...
    SERIALIZE_METHODS(CNewAsset, obj)
    {
        ...
        READWRITE(obj.units);
        READWRITE(obj.nReissuable);
        READWRITE(obj.nHasIPFS);
        if (obj.nHasIPFS == 1) {
            ReadWriteAssetHash(s, ser_action, obj.strIPFSHash);
        }
    }
};
...
public:
    ...
    SERIALIZE_METHODS(CAssetTransfer, obj)
    {
        READWRITE(obj.strName);
        READWRITE(obj.nAmount);
        bool validIPFS = ReadWriteAssetHash(s, ser_action, obj.message);
        if (validIPFS) {
            if constexpr (ser_action.ForRead()) {
                if (!s.empty() && s.size() >= sizeof(int64_t)) {
                    ::Unserialize(s, obj.nExpireTime);
                }
            } else {
                if (obj.nExpireTime != 0) {
                    ::Serialize(s, obj.nExpireTime);
                }
            }
        }
