...
// Assets and messaging/restricted activate at their activation heights on
// mainnet/testnet and are active immediately on regtest.
...
{
    // Asset amounts are stored as CAmount in 1e-8 base units (like SATOX).
...
static const std::regex SATOXCOIN_NAMES("^RVN$|^SATOXCOIN$|^#RVN$|^#SATOXCOIN$");
...
bool IsRootNameValid(const std::string& name)
{
    return std::regex_match(name, ROOT_NAME_CHARACTERS)
        ...
        && !std::regex_match(name, SATOXCOIN_NAMES);
}
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(SATOX_N); // r
    vchMessage.push_back(SATOX_E); // v
    vchMessage.push_back(SATOX_X); // n
    vchMessage.push_back(SATOX_Q); // q

    vchMessage.insert(vchMessage.end(), UCharCast(ssAsset.data()), UCharCast(ssAsset.data() + ssAsset.size()));
    script << OP_SATOX_ASSET << ToByteVector(vchMessage) << OP_DROP;
}
...
void CNewAsset::ConstructOwnerTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(SATOX_E); // v
    vchMessage.push_back(SATOX_X); // n
    vchMessage.push_back(SATOX_O); // o
...
bool OwnerAssetFromScript(const CScript& scriptPubKey, std::string& assetName, std::string& strAddress)
{
    ...
    DataStream ssOwner{std::span{vchOwnerAsset}};
...
bool ReissueAssetFromScript(const CScript& scriptPubKey, CReissueAsset& reissue, std::string& strAddress)
{
    ...
    DataStream ssReissue{std::span{vchReissueAsset}};
...
bool AssetNullDataFromScript(const CScript& scriptPubKey, CNullAssetTxData& assetData, std::string& strAddress)
{
    ...
    DataStream ssData{std::span{vchAssetData}};
...
bool CTransaction::VerifyNewAsset(std::string& strError) const {
    // Issuing an Asset must contain at least 3 CTxOut( Satoxcoin Burn Tx, Any Number of other Outputs ..., Owner Asset Tx, New Asset Tx)
    if (vout.size() < 3) {
    ...
    for (auto out : vout) {
        if (GetHash().GetHex() == BAD_HASH || GetHash().GetHex() == BAD_HASH2) {
...
{
    // Issuing an Asset must contain at least 2 CTxOut( Satoxcoin Burn Tx, New Asset Tx, Any Number of other Outputs...)
    if (vout.size() < 2) {
...
bool CTransaction::VerifyNewRestrictedAsset(std::string& strError) const {
    // Issuing a restricted asset must cointain at least 4 CTxOut(Satoxcoin Burn Tx, Asset Creation, Root Owner Token Transfer, and CNullAssetTxVerifierString)
    if (vout.size() < 4) {
...
{
    // Reissuing an Asset must contain at least 3 CTxOut ( Satoxcoin Burn Tx, Any Number of other Outputs ..., Reissue Asset Tx, Owner Asset Change Tx)
    if (vout.size() < 3) {
...
void CAssetTransfer::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(SATOX_E); // v
    vchMessage.push_back(SATOX_X); // n
    vchMessage.push_back(SATOX_T); // t
...
bool CAssetsCache::UndoAssetCoin(const Coin& coin, const COutPoint& out)
{
    ...
    if (assetName == "" || strAddress == "" || nAmount == 0) {
            LogError("%s : AssetName, Address or nAmount is invalid., Asset Name: %s, Address: %s, Amount: %d", __func__, assetName, strAddress, nAmount); return false;
        }
    if (!AddBackSpentAsset(coin, assetName, strAddress, nAmount, out)) {
            LogError("%s : Failed to add back the spent asset. OutPoint : %s", __func__, out.ToString()); return false;
        }
    return true;
}
...
bool CAssetsCache::RemoveNewAsset(const CNewAsset& asset, const std::string address)
{
    if (!CheckIfAssetExists(asset.strName)) {
            LogError("%s : Tried removing an asset that didn't exist. Asset Name : %s", __func__, asset.strName); return false;
        }
...
{
    if (CheckIfAssetExists(asset.strName)) {
        LogError("%s: Tried adding new asset, but it already existed in the set of assets: %s", __func__, asset.strName);
        return false;
    }
...
bool CAssetsCache::RemoveTransfer(const CAssetTransfer &transfer, const std::string &address, const COutPoint &out)
{
    if (!UndoTransfer(transfer, address, out)) {
            LogError("%s : Failed to undo the transfer", __func__); return false;
        }
...
bool CAssetsCache::Flush()
{
    ...
    if (!passets) {
            LogError("%s: Couldn't find passets pointer while trying to flush assets cache", __func__); return false;
        }
    try {
...
bool CAssetsCache::CheckIfAssetExists(const std::string& name, bool fForceDuplicateCheck)
{
    ...
    if (setNewAssetsToAdd.count(cachedAsset)) {
        if (fForceDuplicateCheck) {
            ...
            LogInfo("%s : Found asset %s in setNewAssetsToAdd but force duplicate check wasn't true\n", __func__, name);
        }
    }
    ...
    if (passetsCache) {
        if (passetsCache->Exists(name)) {
            if (fForceDuplicateCheck) {
                ...
                LogInfo("%s : Found asset %s in passetsCache but force duplicate check wasn't true\n", __func__, name);
            }
        } else {
            if (passetsdb) {
                CNewAsset readAsset;
                ...
                if (passetsdb->ReadAssetData(name, readAsset, nHeight, hash)) {
                    ...
                    if (fForceDuplicateCheck) {
                        ...
                        LogInfo("%s : Found asset %s in passetsdb but force duplicate check wasn't true\n", __func__, name);
                    }
                }
            }
        }
...
bool CAssetsCache::GetAssetMetaDataIfExists(const std::string &name, CNewAsset &asset, int& nHeight, uint256& blockHash)
{
    ...
    // Check the dirty caches first and see if it was recently added or removed
    if (setNewAssetsToRemove.count(cachedAsset)) {
        LogInfo("%s : Found in new assets to Remove - Returning False\n", __func__);
        return false;
    }
    ...
    LogInfo("%s : Didn't find asset meta data anywhere. Returning False\n", __func__);
    return false;
}
...
bool GetAssetData(const CScript& script, CAssetOutputEntry& data)
{
    ...
    if (type == TX_NEW_ASSET && !fIsOwner) {
        ...
        if (TransferAssetFromScript(script, transfer, address)) {
            ...
            LogInfo("Failed to get transfer from script\n");
        }
        ...
        if (nType == TX_NEW_ASSET) {
            if (fIsOwner) {
                if (OwnerAssetFromScript(scriptPubKey, assetName, _strAddress)) {
                    ...
                    LogInfo("%s : Couldn't get new owner asset from script: %s", __func__, HexStr(scriptPubKey));
                }
            } else {
                CNewAsset asset;
                if (AssetFromScript(scriptPubKey, asset, _strAddress)) {
                    ...
                    LogInfo("%s : Couldn't get new asset from script: %s", __func__, HexStr(scriptPubKey));
                }
            }
        } else if (nType == TX_REISSUE_ASSET) {
            CReissueAsset asset;
            if (ReissueAssetFromScript(scriptPubKey, asset, _strAddress)) {
                ...
                LogInfo("%s : Couldn't get reissue asset from script: %s", __func__, HexStr(scriptPubKey));
            }
        } else if (nType == TX_TRANSFER_ASSET) {
            ...
            if (TransferAssetFromScript(scriptPubKey, asset, _strAddress)) {
                ...
                LogInfo("%s : Couldn't get transfer asset from script: %s", __func__, HexStr(scriptPubKey));
            }
        } else {
            LogInfo("%s : Unsupported asset type: %s", __func__, nType);
        }
    } else {
//        LogInfo("%s : Found no asset in script: %s", __func__, HexStr(scriptPubKey));
    }
    if (isAsset) {
//        LogInfo("%s : Found assets in script at address %s : %s (%s)", __func__, _strAddress, assetName, assetAmount);
...
void CNullAssetTxData::ConstructTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), UCharCast(ssAssetTxData.data()), UCharCast(ssAssetTxData.data() + ssAssetTxData.size()));
...
void CNullAssetTxData::ConstructGlobalRestrictionTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), UCharCast(ssAssetTxData.data()), UCharCast(ssAssetTxData.data() + ssAssetTxData.size()));
    script << OP_SATOX_ASSET << OP_RESERVED << OP_RESERVED << ToByteVector(vchMessage);
}
...
void CNullAssetTxVerifierString::ConstructTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), UCharCast(ssAssetTxData.data()), UCharCast(ssAssetTxData.data() + ssAssetTxData.size()));
    script << OP_SATOX_ASSET << OP_RESERVED << ToByteVector(vchMessage);
}
