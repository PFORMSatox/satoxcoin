...
CAssetsDB::CAssetsDB(size_t nCacheSize, bool fMemory, bool fWipe) : CDBWrapper(DBParams{.path = gArgs.GetDataDirNet() / "assets", .cache_bytes = nCacheSize, .memory_only = fMemory, .wipe_data = fWipe}) {
}
