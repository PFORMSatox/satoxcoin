...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Satoxcoin
    //   new: C:\Users\Username\AppData\Local\Satoxcoin
    // macOS: ~/Library/Application Support/Satoxcoin
    // Unix-like: ~/.satoxcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Satoxcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Satoxcoin";
#else
    fs::path pathRet;
