...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // satoxcoin.conf file just ignores the other file.)
        const fs::path orig_datadir_path{args.GetDataDirBase()};
        const fs::path orig_config_path{AbsPathForConfigVal(args, args.GetPathArg("-conf", SATOXCOIN_CONF_FILENAME), /*net_specific=*/false)};
        ...
        // Show an error or warn/log if there is a satoxcoin.conf file in the
        // datadir that is being ignored.
        const fs::path base_config_path = base_path / SATOXCOIN_CONF_FILENAME;
        if (fs::exists(base_config_path)) {
            if (orig_config_path.empty()) {
                LogInfo(
                    ...
                    fs::quoted(SATOXCOIN_CONF_FILENAME));
