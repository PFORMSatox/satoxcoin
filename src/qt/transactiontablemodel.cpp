...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, SatoxcoinUnits::SeparatorStyle separators) const
{
    ...
    switch(wtx->type) {
        ...
        default:
            {
            str = SatoxcoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit,
                                             false, separators);
            } break;
    }
    if (showUnconfirmed) {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, SatoxcoinUnits::separatorAlways);
        case AssetName:
            if (rec->assetName != "RVN")
               return QString::fromStdString(rec->assetName);
            else
               return QString(SatoxcoinUnits::name(walletModel->getOptionsModel()->getDisplayUnit()));
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, SatoxcoinUnits::separatorNever));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, SatoxcoinUnits::separatorNever);
    case AssetNameRole:
        {
            QString assetName;
            if (rec->assetName != "RVN")
               ...
               assetName.append(QString(SatoxcoinUnits::name(walletModel->getOptionsModel()->getDisplayUnit())));
            return assetName;
        }
    case StatusRole:
