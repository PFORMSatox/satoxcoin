...
const char* BIP71_MIMETYPE_PAYMENT = "application/satoxcoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/satoxcoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/satoxcoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the satoxcoin: URI contains a payment request, we are not able to detect the
        ...
        if (arg.startsWith(SATOXCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // satoxcoin: URI
        {
            ...
            if (GUIUtil::parseSatoxcoinURI(arg, &r) && !r.address.isEmpty())
            {
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start satoxcoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling satoxcoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in satoxcoin: URIs
    ...
    if (s.startsWith(SATOXCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // satoxcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseSatoxcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Satoxcoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom satoxcoin addresses are not supported
        ...
        // Satoxcoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(SatoxcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
