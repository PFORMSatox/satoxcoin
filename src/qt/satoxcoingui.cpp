...
const QString SatoxcoinGUI::DEFAULT_WALLET = "~Default";
...
static bool ThreadSafeMessageBox(SatoxcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

SatoxcoinGUI::SatoxcoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    openAction->setStatusTip(tr("Open a satoxcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Satoxcoin command-line options").arg(tr(PACKAGE_NAME)));
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* satoxcoinLabelLayout = new QVBoxLayout(toolbarWidget);
        satoxcoinLabelLayout->addWidget(labelToolbar);
        satoxcoinLabelLayout->addWidget(m_toolbar);
        satoxcoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        satoxcoinLabelLayout->addStretch(1);
        ...
        // Get the latest Satoxcoin release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void SatoxcoinGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/rvntext")));
        m_toolbar->setMaximumWidth(65);
...
void SatoxcoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool SatoxcoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool SatoxcoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SatoxcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SatoxcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SatoxcoinGUI::showWalletRepair()
{
...
void SatoxcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Satoxcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void SatoxcoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}
...
void SatoxcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void SatoxcoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Satoxcoin"); // default title
...
#ifdef ENABLE_WALLET
void SatoxcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    ...
    if (assetName == "RVN")
        ...
        msg += tr("Amount: %1\n").arg(SatoxcoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void SatoxcoinGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void SatoxcoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMnemonic(SatoxcoinGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void SatoxcoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void SatoxcoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<SatoxcoinUnits::Unit> units = SatoxcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SatoxcoinUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(SatoxcoinUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(SatoxcoinUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (SatoxcoinUnits::Unit u : SatoxcoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(SatoxcoinUnits::name(u)), this);
...
{
    setText(SatoxcoinUnits::name(newUnits));
}
...
void SatoxcoinGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SatoxcoinGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "SatoxcoinGUI::onPriceUnitChange: " + QString::number(newIndex);
