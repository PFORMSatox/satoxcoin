...
{
    return QCoreApplication::translate("satoxcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. satoxcoin_de.qm (shortcut "de" needs to be defined in satoxcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. satoxcoin_de_DE.qm (shortcut "de_DE" needs to be defined in satoxcoin.qrc)
...
/** Class encapsulating Satoxcoin Core startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class SatoxcoinCore: public QObject
{
    Q_OBJECT
public:
    explicit SatoxcoinCore();
...
class SatoxcoinApplication: public QApplication
{
    Q_OBJECT
public:
    explicit SatoxcoinApplication();
    ~SatoxcoinApplication();
    ...
    /// Get window identifier of QMainWindow (SatoxcoinGUI)
...
#include "satoxcoin.moc"

SatoxcoinCore::SatoxcoinCore():
    QObject()
{
}

void SatoxcoinCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(GetWarnings("gui")));
}

bool SatoxcoinCore::baseInitialize()
{
...
void SatoxcoinCore::restart(QStringList args)
{
...
void SatoxcoinApplication::createOptionsModel(bool resetSettings)
{
...
{
    window = new SatoxcoinGUI(platformStyle, networkStyle, 0);
...
void SatoxcoinApplication::startThread()
{
    ...
    SatoxcoinCore *executor = new SatoxcoinCore();
...
void SatoxcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        if (!vpwallets.empty())
        {
            ...
            window->addWallet(SatoxcoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(SatoxcoinGUI::DEFAULT_WALLET);
        ...
        // satoxcoin: URIs or payment requests:
...
void SatoxcoinApplication::shutdownResult(bool success)
{
    returnValue = success ? EXIT_SUCCESS : EXIT_FAILURE;
...
{
    QMessageBox::critical(0, "Runaway exception", SatoxcoinGUI::tr("A fatal error occurred. Satoxcoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse satoxcoin.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", SATOXCOIN_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(0, QObject::tr(PACKAGE_NAME),
    ...
    // satoxcoin: links repeatedly have their payment requests routed to this process:
    ...
    try
    {
        ...
        if (SatoxcoinCore::baseInitialize()) {
