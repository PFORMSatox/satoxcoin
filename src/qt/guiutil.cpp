...
#include "satoxcoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Satoxcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new SatoxcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new SatoxcoinAddressCheckValidator(parent));
}
...
bool parseSatoxcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no satoxcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("satoxcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!SatoxcoinUnits::parse(SatoxcoinUnits::RVN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseSatoxcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert satoxcoin:// to satoxcoin:
    //
    //    Cannot handle this later, because satoxcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("satoxcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "satoxcoin:");
    }
    QUrl uriInstance(uri);
    return parseSatoxcoinURI(uriInstance, out);
}

QString formatSatoxcoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("satoxcoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SatoxcoinUnits::format(SatoxcoinUnits::RVN, info.amount, false, SatoxcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openSatoxcoinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(SATOXCOIN_CONF_FILENAME);
    ...
    /* Open satoxcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Satoxcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Satoxcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Satoxcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Satoxcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "satoxcoin.desktop";
    return GetAutostartDir() / strprintf("satoxcoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a satoxcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Satoxcoin\n";
        else
            optionFile << strprintf("Name=Satoxcoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the satoxcoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef satoxcoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (satoxcoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, satoxcoinAppUrl);

    CFRelease(satoxcoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add satoxcoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, satoxcoinAppUrl, nullptr, nullptr);
    }
