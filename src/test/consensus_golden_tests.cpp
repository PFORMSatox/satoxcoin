...
// doc/security/golden-consensus-manifest.md. If this test fails, the codebase
// no longer matches the frozen mainnet consensus rules -> do NOT ship.
...
{
    SelectParams(CBaseChainParams::MAIN, true);
    const auto p = CreateChainParams(CBaseChainParams::MAIN);
    ...
    BOOST_CHECK_EQUAL(c.vDeployments[Consensus::DEPLOYMENT_ASSETS].nOverrideRuleChangeActivationThreshold, 1814);
    BOOST_CHECK_EQUAL(c.vDeployments[Consensus::DEPLOYMENT_MSG_REST_ASSETS].nOverrideRuleChangeActivationThreshold, 1714);
    BOOST_CHECK_EQUAL(c.vDeployments[Consensus::DEPLOYMENT_ENFORCE_VALUE].nOverrideRuleChangeActivationThreshold, 1411);
    ...
    // Section 1.8: checkpoint count and last checkpoint (height 1865353)
    BOOST_CHECK_EQUAL(p->Checkpoints().mapCheckpoints.size(), (size_t)55);
    BOOST_CHECK(p->Checkpoints().mapCheckpoints.count(1865353) == 1);
    BOOST_CHECK_EQUAL(p->Checkpoints().mapCheckpoints.at(1865353).GetHex(),
                      "00000000342546860cd9250210c63c000e67b5fe1b26559fe5e86b4052175c38");
