...
// Regression tests for the KAWPOW security fixes (2026-08-13 incident):
//  1. mix_hash forgery - the vulnerable code path (progpow::hash_no_verify)
//     which computes the final hash FROM the attacker-supplied mix_hash without
//     verifying it. An attacker could forge mix_hash and pass PoW without doing
//     any real DAG/GPU work.
//  2. block.nHeight (serialized in the KAWPOW header) was never checked against
//     the chain height. Since the DAG epoch is derived from block.nHeight, an
//     attacker could set a low height to select a tiny/cheap epoch and satisfy
//     PoW with negligible work.
// These tests assert the hashing-level properties the fixes rely on.
BOOST_AUTO_TEST_CASE(kawpow_mixhash_forgery_regression)
{
    // Property 1: a forged mix_hash changes the final hash. The vulnerable path
    // (progpow::hash_no_verify) derives the final hash from the provided mix_hash,
    // so an attacker who controls mix_hash controls the final hash. progpow::hash
    // (full verification) derives mix_hash from the DAG; it is an input, not an
    // output of the attack. Verify that the two diverge for a forged mix_hash,
    // which is exactly why the full path must be used (mix_hash must be checked).
    const int block_number = 30000;
    const auto header =
            to_hash256("ffeeddccbbaa9988776655443322110000112233445566778899aabbccddeeff");
    const uint64_t nonce = 0x123456789abcdef0;
    ...
    // A forged mix_hash yields a different final hash from the honest computation.
    auto forged_mix = result.mix_hash;
    ++forged_mix.bytes[7];
    const auto forged_final = progpow::hash_no_verify(
            block_number, header, forged_mix, nonce);
    BOOST_CHECK(to_hex(forged_final) != to_hex(result.final_hash));

    // The honest mix_hash recomputes to the identical final hash via
    // hash_no_verify - i.e. verification must check mix_hash, not just the final
    // hash, otherwise the attacker controls the output.
    const auto honest_final = progpow::hash_no_verify(
            block_number, header, result.mix_hash, nonce);
    BOOST_CHECK_EQUAL(to_hex(honest_final), to_hex(result.final_hash));

    // Property 2: the KAWPOW hash depends on the block height (epoch). A forged
    // lower height selects a different epoch and produces a different mix/final
    // hash - proving the height check is what prevents choosing a cheap epoch.
    const auto forged_height_result = progpow::hash(
            *context, block_number - ethash::epoch_length, header, nonce);
    BOOST_CHECK(to_hex(forged_height_result.mix_hash) != to_hex(result.mix_hash));
    BOOST_CHECK(to_hex(forged_height_result.final_hash) != to_hex(result.final_hash));
}
