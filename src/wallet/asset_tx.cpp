...
// Copyright (c) 2020-2024 The Satoxcoin Core developers
...
//! Legacy nodes require standard P2PKH (25 bytes) with OP_SATOX_ASSET at index 25 (see CScript::IsAssetScript).
...
//! Generate an OP_SATOX_ASSET <hash> script for null asset data (qualifier tags, restrictions)
static CScript GetScriptForNullAssetDataDestination(const CTxDestination& dest)
{
    CScript script;
    if (auto* pkh = std::get_if<PKHash>(&dest)) {
        script << OP_SATOX_ASSET << ToByteVector(*pkh);
    } else if (auto* sh = std::get_if<ScriptHash>(&dest)) {
        script << OP_SATOX_ASSET << ToByteVector(*sh);
    }
    return script;
}
    ...
    std::string* verifier_string)
{
    ...
    // Allow the wallet to select additional native (SATOX) inputs for fees
    ...
    std::vector<CNullAssetTxData>* nullGlobalRestrictionData)
{
    ...
    if (bal.m_mine_trusted == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any SATOX; transferring an asset requires a network fee"));
        return false;
    }
