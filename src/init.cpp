 ...
 * between satoxcoind, and satoxcoin-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("satoxcoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateSatoxcoins(false, 0, GetParams());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), SATOXCOIN_PID_FILENAME));
#endif
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("satoxcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Satoxcoin is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Satoxcoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", SATOXCOIN_CONF_FILENAME)).string());
    ...
    GenerateSatoxcoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
